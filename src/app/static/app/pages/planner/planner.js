...
function makeChar(){
	...
	Object.keys(getCalculator().CHARACTERS).forEach((name) => {
		...
		const Img = createImg(Char, 'chr-image', getCharacter(name))
		const Name = createTxt(Char, 'div', {'class':'name'}, name);
...
function makeWpn(){
	...
	Object.keys(getCalculator().WEAPONS).forEach((name) => {
		...
		const Img = createImg(Wpn, 'wpn-image', getWeapon(name))
		const Name = createTxt(Wpn, 'div', {'class':'name'}, name)

		let state = uGet(userWpn[name],'')
		makeInputs(Wd, name, 'WEAPONS', 'WFARM', {PHASE: state.PHASE, TARGET: state.TARGET});
...
function makeInputs(Root, name, kind, farm, uValues){
	...
	Object.entries(uValues).forEach(([uAttribute, uValue],i) => {
...
function makeFarm(Root, name, kind, farm){
	...
	Object.entries(calculator[kind][name][farm]).forEach(([cCategory, [cItem, cMaterials]]) => {
		...
		Object.entries(cMaterials).reverse().forEach(([rank, value]) => {
			...
			if(value === 0) return;
			Div.addEventListener('mouseover', ()=>tooltip.show(cItem))
			Div.addEventListener('mouseout', ()=>tooltip.hide())
	
			const Card = create(Div, 'div', {'class':'item r_'+rank})

			const Img = createImg(Card, 'item__image', getImage(category, item, rank))
			const NEED = createTxt(Card, 'p', {'class':'item__need'}, value)
		});
		if(counter > 1){
